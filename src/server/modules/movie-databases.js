  ...
  static theMovieDB(movieTitle, year = new Date().getFullYear()) {
    ...
    return new Promise((resolve, reject) => {
      const theMovieDbOptions = {
        uri: `${theMovieDbBaseUrl}/search/movie?query=${movieTitle}&api_key=${process
          .env.THE_MOVIE_DB_API_KEY}&year=${year}`,
        json: true
      };
      ...
      rpn(theMovieDbOptions)
        .then(result => {
          if (result.results.length > 0) {
            ...
            theMovieDbOptions.uri = `${theMovieDbBaseUrl}/movie/${bestMatchingMovie.id}/videos?api_key=${process
              .env.THE_MOVIE_DB_API_KEY}&language=en-US`;
            rpn(theMovieDbOptions)
              .then(videoData => {
              ...
              */
                ...
                theMovieDbOptions.uri = `${theMovieDbBaseUrl}/movie/${bestMatchingMovie.id}?api_key=${process
                  .env.THE_MOVIE_DB_API_KEY}&language=en-US`;
              ...
              .then(movieData => {
                ...
                // now get the actors
                theMovieDbOptions.uri = `${theMovieDbBaseUrl}/movie/${bestMatchingMovie.id}/credits?api_key=${process
                  .env.THE_MOVIE_DB_API_KEY}&language=en-US`;
                return rpn(theMovieDbOptions);
              })
              .then(actorData => {
      ...
      rpn(options)
        ...
        .then($ => {
          ...
          if (!rottenTomatoesMatch) {
            ...
            rpn(options).then(rtData => {
              resolve({
                rottenTomatoesUrl,
                tomatoMeter: rtData(
                  '#all-critics-numbers .meter-value span'
                ).text(),
                tomatoConsensus: rtData(
                  '#all-critics-numbers .critic_consensus'
                )
                  .text()
